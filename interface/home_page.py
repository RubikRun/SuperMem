...
class HomePage:
    ...
    def do_word_test(self) -> None:
        ...
        # Get word indices in the requested order
        word_idxs = self.get_words_ordered_in_mode(dictionary.words[:words_count], order_mode, self.user.confidences[language_idx])
    ...
    # Orders words in the given mode.
    # Returns a list of indices to the words in the original list.
    # Does not modify the original list.
    def get_words_ordered_in_mode(self, words: list[Word], mode: int, confidences: list[int]) -> list[int]:
        # Initialize none indices
        word_idxs = None
        # Get the indices based on the mode
        if mode == 1:
            # Sort words by their level, ascending
            word_idxs = [tup[0] for tup in sorted(enumerate(words), key=lambda x:x[1].level)]
        elif mode == 2:
            # Sort words by their level, descending
    ...
    # How much the array is shuffled is determined by the randomness parameter, call it R.
    # R ranges from 0 to 1. R=0 means no shuffle. R=1 means full shuffle (not mathematically correc).
    # The shuffle is done by performing random swaps of elements
    def shuffle_a_bit(array: list, randomness: float) -> list:
        length = len(array)
